...
# Save plot in folder
ggsave("Output/Plots/Regressions/logKoa_logku3.png", plot = plot.ku,
...
# Save plot in folder
ggsave("Output/Plots/Regressions/DPCBAir_logku3.png", plot = plot.ku2,
       width = 5, height = 5, dpi = 500)
...
# Save plot in folder
ggsave("Output/Plots/Regressions/logKoa_logke3.png", plot = plot.ke,
       width = 5, height = 5, dpi = 500)
...
plot.kpan <- ggplot(combined_df, aes(x = logKoa, y = logKpan)) +
  ...
  annotate("text", x = min(combined_df$logKoa), 
           ...
           label = eq_text, hjust = 0, vjust = 1, size = 4) +
  annotate("text", x = min(combined_df$logKoa), 
           y = max(combined_df$logKpan) - 0.05, 
           label = R2_text, hjust = 0, vjust = 1, size = 4) +
  annotate("text", x = min(combined_df$logKoa), 
           y = max(combined_df$logKpan) - 0.1, 
           label = p_text, hjust = 0, vjust = 1, size = 4) +
  theme_bw() +
  labs(x = expression(bold("log Koa")),
       y = bquote(bold("log Kpan"))) +
...
# Save plot in folder
ggsave("Output/Plots/Regressions/logKao_logKpan3.png", plot = plot.kpan,
       width = 5, height = 5, dpi = 500)
...
# Save plot in folder
ggsave("Output/Plots/Regressions/logKao_Rs2.png", plot = plot.Rs,
